...
public class PongBallFour : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Segurança: desativa gravidade e evita rotação indesejada
        rb.gravityScale = 0f;
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (other.gameObject.CompareTag("GoalZoneL"))
        {
            ...
            if (server != null) server.AddScore(false); // false = time esquerda levou gol
            ResetBall();
        }
        ...
        else if (other.gameObject.CompareTag("GoalZoneR"))
        {
            ...
            if (server != null) server.AddScore(true); // true = time direita levou gol
            ResetBall();
        }
    }
